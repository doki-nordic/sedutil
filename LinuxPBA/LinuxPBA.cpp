...
//    system ("tput clear");
    ...
    bool escape;
    string p = GetPassPhrase("Please enter pass-phrase to unlock OPAL drives: ", escape);
    UnlockSEDs((char *)p.c_str(), argc > 1 && p != "" ? argv[1] : NULL);
    if (escape) {
        printf("Restarting system\n");
        sync();
        usleep(300000); // give the user time to see results
        reboot(RB_AUTOBOOT);
    }
    return 0;
}
